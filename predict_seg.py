...
# 设置输入目录和输出目录
input_dir = "data/real_map/"  # 输入图片所在目录
output_dir = "output_masks/"  # 输出掩码图片保存目录
...
for filename in image_files:
    ...
    for result in results:
        for i, mask in enumerate(result.masks.data):
            ...
            cv2.putText(img, class_name, (int(x_min), int(y_min) - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, text_color, 2, cv2.LINE_AA)
